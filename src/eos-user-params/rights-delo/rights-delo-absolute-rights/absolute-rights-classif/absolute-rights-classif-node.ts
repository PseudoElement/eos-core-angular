...
export class RightClassifNode {
    ...
    constructor(item: ITechUserClassifConst, user: IParamUserCl, pNode: NodeAbsoluteRight, component: AbsoluteRightsClassifComponent) {
        ...
        if ((this._item.expandable !== E_TECH_USER_CLASSIF_CONTENT.none) && this._curentUser['USER_TECH_List'].length) {
           this._listUserTech = this._curentUser['USER_TECH_List'].filter((i: USER_TECH) => i['FUNC_NUM'] === this.key);
        }
        if (this._item.expandable !== E_TECH_USER_CLASSIF_CONTENT.none) {
            this._config = this._component.getConfig(this._item.expandable);
        }
    ...
    addInstance() {
        // this.isShell = true;
        this._component.addInstance(this._config);
    }
    DeleteInstance() {
