...
export class RightsDeloCardIndexRightsComponent implements OnInit {
    ...
    if (newArrayDataDocumentsForMerge !== null) { // Если пользователь захотел убрать checkbox 'Все группы документов'
    loop1:
                ...
                if (newArrayDataDocumentsForMerge[t]['DUE_CARD'] === newDataFromLocalStorageFuncFileCards[s][0]) {
                    for (let e = 0; e < newDataFromLocalStorageFuncFileCards[s][1]['USER_CARD_DOCGROUP_List'].length; e++) {
                        ...
                        /*  if (newArrayDataDocumentsForDelete !== null) {
                                ...
                                for (let f = 0; f < newArrayDataDocumentsForDelete.length; f++) {
                                    if (newArrayDataDocumentsForDelete[f]['FUNC_NUM'] === 1) {
                                        if (newDataFromLocalStorageFuncFileCards[s][1]['USER_CARD_DOCGROUP_List'][e]['DUE'] === newArrayDataDocumentsForDelete[f]['DUE']) {
                                            this.flagEmptyAllowedCreateRofD = false;
                                            if ((newArrayDataDocumentsForDelete.length - 1) === f && !this.flagEmptyAllowedCreateRofD) {
                                                this.msgSrv.addNewMessage(EMPTY_ALLOWED_CREATE_REGISTRATION_OF_DOCUMENTS);
                                                return [];
                                            }
                                    } else if ((newArrayDataDocumentsForDelete.length - 1) === f && this.flagEmptyAllowedCreateRofD) {
