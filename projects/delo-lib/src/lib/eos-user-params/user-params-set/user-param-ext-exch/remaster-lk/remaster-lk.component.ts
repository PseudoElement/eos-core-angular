...
@Component({
    ...
    templateUrl: 'remaster-lk.component.html',
    providers: [FormHelperService],
})
export class RemasterLkComponent implements OnInit, OnDestroy, AfterViewInit {
    public fieldsConst: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_LK));
    public fieldsConstMailResive: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_MAILRESIVE_LK));
    ...
    private setRcSend = new Set()
        .add('RCSEND_LK_FOR_MULTIPOINT_DOCUMENTS_SEND_RADIO')
        .add('RCSEND_LK_RESOLUTIONS_RADIO')
        .add('RCSEND_LK_ADDRESSEES_RADIO');
    ...
    private setMailResive = new Set()
        .add('MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO')
        .add('MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO');
    ...
    ngAfterViewInit() {
        ...
        this._searchService.emailExtChangeObservable.pipe(
            takeUntil(this.ngUnsubscribe)
        )
    ...
    ngOnInit() {
        ...
        this.stringRCSEND = this.userData['RCSEND_LK'];
        this.stringMailResive = this.userData['MAILRECEIVE_LK'];
        this.mapDefault = {
            RCSEND_LK: this.defaultValues['RCSEND_LK'],
            MAILRECEIVE_LK: this.defaultValues['MAILRECEIVE_LK'],
        };
    }
    ...
    initEmail() {
        this.preparedItemForInputs = this.parse_Create(this.fieldsConst.fields, 'userData', 'RCSEND_LK');
    ...
    initMailResive() {
        this.preparedItemForInputsMailREsive = this.parse_Create(this.fieldsConstMailResive.fields, 'userData', 'MAILRECEIVE_LK');
    ...
    returnClass(node, form: FormGroup): string {
        ...
        if (key === 'rec.RCSEND_LK_REGISTRATION_NUMBER') {
            key = 'rec.RCSEND_LK_DOCUMENT_AUTHOR';
        }
        if (node.isOpen) {
    ...
    subscriberFormRcSend() {
        this.form.valueChanges.subscribe(data => {
            ...
            if (dataVal['RCSEND_LK'] !== this.stringRCSEND || this.mapNewValue.size > 0) {
    ...
    triggerMailReceive(node: TreeItem) {
        if (node.key === 'MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT' || node.key === 'MAILRECEIVE_LK_TAKE_RUBRICS_RK') {
    ...
    enabelDisabelMailResive(tree: TreeItem) {
        ...
        if (tree.key === 'MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable();
            }
        }
        if (tree.key === 'MAILRECEIVE_LK_TAKE_RUBRICS_RK') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO'].disable();
            }
        }
    }
    ...
    setParent(node: TreeItem, val?) {
        ...
        if (node.key === 'RCSEND_LK_ADDRESSEES' || node.key === 'RCSEND_LK_RESOLUTIONS') {
            this.enabelDisabelRcSend(node);
        }
    }

    disableForm() {
        const addresInput = this.form.controls['rec.RCSEND_LK_ADDRESSEES'].value;
        const resolutionInput = this.form.controls['rec.RCSEND_LK_RESOLUTIONS'].value;

        (!addresInput) ? this.form.controls['rec.RCSEND_LK_ADDRESSEES_RADIO'].disable({ emitEvent: false }) :
                         this.form.controls['rec.RCSEND_LK_ADDRESSEES_RADIO'].enable({ emitEvent: false });

        (!resolutionInput) ? this.form.controls['rec.RCSEND_LK_RESOLUTIONS_RADIO'].disable({ emitEvent: false }) :
                             this.form.controls['rec.RCSEND_LK_RESOLUTIONS_RADIO'].enable({ emitEvent: false });
    }

    disableFormMailResive() {
        const addresInput = this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT'].value;
        const resolutionInput = this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK'].value;

        (!addresInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable({ emitEvent: false }) :
                        this.formMailResuve.controls['rec.MAILRECEIVE_LK_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable({ emitEvent: false });
        
        (!resolutionInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO'].disable({ emitEvent: false }) : 
                             this.formMailResuve.controls['rec.MAILRECEIVE_LK_TAKE_RUBRICS_RK_RADIO'].enable({ emitEvent: false });
    }

    getDefaultValue(key: string, type: 'rcSend' | 'mailReceive') {
        if (type === 'rcSend') {
            if (!this.prepareDefaultForm) {
                this.prepareDefaultForm = this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_LK');
            }
            return this.prepareDefaultForm[key].toString();
        }
        if (type === 'mailReceive') {
            if (!this.prepareDefaultFormMailREceive) {
                this.prepareDefaultFormMailREceive = this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_LK');
            }
    ...
    enabelDisabelRcSend(tree: TreeItem) {
        ...
        if (tree.key === 'RCSEND_LK_ADDRESSEES') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_LK_ADDRESSEES_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_LK_ADDRESSEES_RADIO'].enable();
                this.form.controls['rec.RCSEND_LK_ADDRESSEES_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_LK_ADDRESSEES_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_LK_ADDRESSEES_RADIO'].disable();
            }
        }
        if (tree.key === 'RCSEND_LK_RESOLUTIONS') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_LK_RESOLUTIONS_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_LK_RESOLUTIONS_RADIO'].enable();
                this.form.controls['rec.RCSEND_LK_RESOLUTIONS_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_LK_RESOLUTIONS_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_LK_RESOLUTIONS_RADIO'].disable();
            }
        }
    }
    ...
    subscriberFormMailResive() {
        this.formMailResuve.valueChanges.subscribe(data => {
            ...
            const dataVal = this.userData !== null ? this.userData : this.defaultValues;
            if (dataVal['MAILRECEIVE_LK'] !== this.stringMailResive) {
    ...
    emitChange() {
        const obj = {
            'rec.RCSEND_LK': this.stringRCSEND,
            'rec.MAILRECEIVE_LK': this.stringMailResive,
        };
    ...
    default() {
        this.prepareDefaultForm = this.prepareDefaultForm ? this.prepareDefaultForm : this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_LK');
        this.prepareDefaultFormMailREceive = this.prepareDefaultFormMailREceive ? this.prepareDefaultFormMailREceive : this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_LK');
