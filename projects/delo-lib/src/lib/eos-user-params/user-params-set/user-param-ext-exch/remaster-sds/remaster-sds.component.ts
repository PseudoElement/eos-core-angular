...
@Component({
    ...
    templateUrl: 'remaster-sds.component.html',
    providers: [FormHelperService],
})
export class RemasterSdsComponent implements OnInit, OnDestroy, AfterViewInit {
    public fieldsConst: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_SDS)) ;
    public fieldsConstMailResive: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_MAILRESIVE_SDS));
    ...
    private setRcSend = new Set()
        .add('RCSEND_SDS_FOR_MULTIPOINT_DOCUMENTS_SEND_RADIO')
        .add('RCSEND_SDS_RESOLUTIONS_RADIO')
        .add('RCSEND_SDS_ADDRESSEES_RADIO');
    ...
    private setMailResive = new Set()
        .add('MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO')
        .add('MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO');
    ...
    ngOnInit() {
        ...
        this.stringRCSEND = this.userData['RCSEND_SDS'];
        this.stringMailResive = this.userData['MAILRECEIVE_SDS'];
        this.mapDefault = {
            RCSEND_SDS: this.defaultValues['RCSEND_SDS'],
            MAILRECEIVE_SDS: this.defaultValues['MAILRECEIVE_SDS'],
        };
    }
    ...
    initEmail() {
        this.preparedItemForInputs = this.parse_Create(this.fieldsConst.fields, 'userData', 'RCSEND_SDS');
    ...
    initMailResive() {
        this.preparedItemForInputsMailREsive = this.parse_Create(this.fieldsConstMailResive.fields, 'userData', 'MAILRECEIVE_SDS');
    ...
    returnClass(node, form: FormGroup): string {
        ...
        if (key === 'rec.RCSEND_SDS_REGISTRATION_NUMBER') {
            key = 'rec.RCSEND_SDS_DOCUMENT_AUTHOR';
        }
        if (node.isOpen) {
    ...
    subscriberFormRcSend() {
        this.form.valueChanges.subscribe(data => {
            ...
            if (dataVal['RCSEND_SDS'] !== this.stringRCSEND || this.mapNewValue.size > 0) {
    ...
    triggerMailReceive(node: TreeItem) {
        if (node.key === 'MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT' || node.key === 'MAILRECEIVE_SDS_TAKE_RUBRICS_RK') {
    ...
    enabelDisabelMailResive(tree: TreeItem) {
        ...
        if (tree.key === 'MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable();
            }
        }
        if (tree.key === 'MAILRECEIVE_SDS_TAKE_RUBRICS_RK') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO'].disable();
            }
        }
    }
    ...
    setParent(node: TreeItem, val?) {
        ...
        if (node.key === 'RCSEND_SDS_ADDRESSEES' || node.key === 'RCSEND_SDS_RESOLUTIONS') {
            this.enabelDisabelRcSend(node);
        }
    }

    disableForm() {
        const addresInput = this.form.controls['rec.RCSEND_SDS_ADDRESSEES'].value;
        const resolutionInput = this.form.controls['rec.RCSEND_SDS_RESOLUTIONS'].value;

        (!addresInput) ? this.form.controls['rec.RCSEND_SDS_ADDRESSEES_RADIO'].disable({ emitEvent: false }) :
                         this.form.controls['rec.RCSEND_SDS_ADDRESSEES_RADIO'].enable({ emitEvent: false });

        (!resolutionInput) ? this.form.controls['rec.RCSEND_SDS_RESOLUTIONS_RADIO'].disable({ emitEvent: false }) :
                             this.form.controls['rec.RCSEND_SDS_RESOLUTIONS_RADIO'].enable({ emitEvent: false });
    }

    disableFormMailResive() {
        const addresInput = this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT'].value;
        const resolutionInput = this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK'].value;

        (!addresInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable({ emitEvent: false }) :
                        this.formMailResuve.controls['rec.MAILRECEIVE_SDS_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable({ emitEvent: false });
        
        (!resolutionInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO'].disable({ emitEvent: false }) : 
                             this.formMailResuve.controls['rec.MAILRECEIVE_SDS_TAKE_RUBRICS_RK_RADIO'].enable({ emitEvent: false });
    }

    getDefaultValue(key: string, type: 'rcSend' | 'mailReceive') {
        if (type === 'rcSend') {
            if (!this.prepareDefaultForm) {
                this.prepareDefaultForm = this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_SDS');
            }
        ...
        if (type === 'mailReceive') {
            if (!this.prepareDefaultFormMailREceive) {
                this.prepareDefaultFormMailREceive = this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_SDS');
            }
    ...
    enabelDisabelRcSend(tree: TreeItem) {
        ...
        if (tree.key === 'RCSEND_SDS_ADDRESSEES') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_SDS_ADDRESSEES_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_SDS_ADDRESSEES_RADIO'].enable();
                this.form.controls['rec.RCSEND_SDS_ADDRESSEES_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_SDS_ADDRESSEES_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_SDS_ADDRESSEES_RADIO'].disable();
            }
        }

        if (tree.key === 'RCSEND_SDS_RESOLUTIONS') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_SDS_RESOLUTIONS_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_SDS_RESOLUTIONS_RADIO'].enable();
                this.form.controls['rec.RCSEND_SDS_RESOLUTIONS_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_SDS_RESOLUTIONS_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_SDS_RESOLUTIONS_RADIO'].disable();
            }
        }
    }
    ...
    subscriberFormMailResive() {
        this.formMailResuve.valueChanges.subscribe(data => {
            ...
            const dataVal = this.userData !== null ? this.userData : this.defaultValues;
            if (dataVal['MAILRECEIVE_SDS'] !== this.stringMailResive) {
    ...
    emitChange() {
        const obj = {
            'rec.RCSEND_SDS': this.stringRCSEND,
            'rec.MAILRECEIVE_SDS': this.stringMailResive,
        };
    ...
    default() {
        this.prepareDefaultForm = this.prepareDefaultForm ? this.prepareDefaultForm : this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_SDS');
        this.prepareDefaultFormMailREceive = this.prepareDefaultFormMailREceive ? this.prepareDefaultFormMailREceive : this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_SDS');
