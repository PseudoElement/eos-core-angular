...
@Component({
    ...
    templateUrl: 'remaster-aspsd.component.html',
    providers: [FormHelperService],
})
export class RemasterAspsdComponent implements OnInit, OnDestroy, AfterViewInit {
    public fieldsConst: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_ASPSD)) ;
    public fieldsConstMailResive: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_MAILRESIVE_ASPSD));
    ...
    private setRcSend = new Set()
        .add('RCSEND_ASPSD_FOR_MULTIPOINT_DOCUMENTS_SEND_RADIO')
        .add('RCSEND_ASPSD_RESOLUTIONS_RADIO')
        .add('RCSEND_ASPSD_ADDRESSEES_RADIO');
    ...
    private setMailResive = new Set()
        .add('MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO')
        .add('MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO');
    ...
    ngOnInit() {
        ...
        this.stringRCSEND = this.userData['RCSEND_ASPSD'];
        this.stringMailResive = this.userData['MAILRECEIVE_ASPSD'];
        this.mapDefault = {
            RCSEND_ASPSD: this.defaultValues['RCSEND_ASPSD'],
            MAILRECEIVE_ASPSD: this.defaultValues['MAILRECEIVE_ASPSD'],
        };
    }
    ...
    initEmail() {
        this.preparedItemForInputs = this.parse_Create(this.fieldsConst.fields, 'userData', 'RCSEND_ASPSD');
    ...
    initMailResive() {
        this.preparedItemForInputsMailREsive = this.parse_Create(this.fieldsConstMailResive.fields, 'userData', 'MAILRECEIVE_ASPSD');
    ...
    returnClass(node, form: FormGroup): string {
        ...
        if (key === 'rec.RCSEND_ASPSD_REGISTRATION_NUMBER') {
            key = 'rec.RCSEND_ASPSD_DOCUMENT_AUTHOR';
        }
        if (node.isOpen) {
    ...
    subscriberFormRcSend() {
        this.form.valueChanges.subscribe(data => {
            ...
            if (dataVal['RCSEND_ASPSD'] !== this.stringRCSEND || this.mapNewValue.size > 0) {
    ...
    triggerMailReceive(node: TreeItem) {
        if (node.key === 'MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT' || node.key === 'MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK') {
    ...
    enabelDisabelMailResive(tree: TreeItem) {
        ...
        if (tree.key === 'MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable();
            }
        }
        if (tree.key === 'MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO'].disable();
            }
        }
    }
    ...
    setParent(node: TreeItem, val?) {
        ...
        if (node.key === 'RCSEND_ASPSD_ADDRESSEES' || node.key === 'RCSEND_ASPSD_RESOLUTIONS') {
            this.enabelDisabelRcSend(node);
        }
    }

    disableForm() {
        const addresInput = this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES'].value;
        const resolutionInput = this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS'].value;

        (!addresInput) ? this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES_RADIO'].disable({ emitEvent: false }) :
                         this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES_RADIO'].enable({ emitEvent: false });

        (!resolutionInput) ? this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS_RADIO'].disable({ emitEvent: false }) :
                             this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS_RADIO'].enable({ emitEvent: false });
    }

    disableFormMailResive() {
        const addresInput = this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT'].value;
        const resolutionInput = this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK'].value;

        (!addresInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable({ emitEvent: false }) :
                        this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable({ emitEvent: false });
        
        (!resolutionInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO'].disable({ emitEvent: false }) : 
                             this.formMailResuve.controls['rec.MAILRECEIVE_ASPSD_TAKE_RUBRICS_RK_RADIO'].enable({ emitEvent: false });
    }

    getDefaultValue(key: string, type: 'rcSend' | 'mailReceive') {
        if (type === 'rcSend') {
            if (!this.prepareDefaultForm) {
                this.prepareDefaultForm = this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_ASPSD');
            }
        ...
        if (type === 'mailReceive') {
            if (!this.prepareDefaultFormMailREceive) {
                this.prepareDefaultFormMailREceive = this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_ASPSD');
            }
    ...
    enabelDisabelRcSend(tree: TreeItem) {
        ...
        if (tree.key === 'RCSEND_ASPSD_ADDRESSEES') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_ASPSD_ADDRESSEES_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES_RADIO'].enable();
                this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_ASPSD_ADDRESSEES_RADIO'].disable();
            }
        }

        if (tree.key === 'RCSEND_ASPSD_RESOLUTIONS') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_ASPSD_RESOLUTIONS_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS_RADIO'].enable();
                this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_ASPSD_RESOLUTIONS_RADIO'].disable();
            }
        }
    }
    ...
    setValueChildren(node: TreeItem, val: boolean) {
        ...
        node.children.forEach((field: TreeItem) => { this.setValueChildren(field, val); });
    }
    ...
    subscriberFormMailResive() {
        this.formMailResuve.valueChanges.subscribe(data => {
            ...
            const dataVal = this.userData !== null ? this.userData : this.defaultValues;
            if (dataVal['MAILRECEIVE_ASPSD'] !== this.stringMailResive) {
    ...
    emitChange() {
        const obj = {
            'rec.RCSEND_ASPSD': this.stringRCSEND,
            'rec.MAILRECEIVE_ASPSD': this.stringMailResive,
        };
    ...
    default() {
        this.prepareDefaultForm = this.prepareDefaultForm ? this.prepareDefaultForm : this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_ASPSD');
        this.prepareDefaultFormMailREceive = this.prepareDefaultFormMailREceive ? this.prepareDefaultFormMailREceive : this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_ASPSD');
