...
@Component({
    ...
    templateUrl: 'remaster-epvv.component.html',
    providers: [FormHelperService],
})
export class RemasterEpvvComponent implements OnInit, OnDestroy, AfterViewInit {
    public fieldsConst: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_EPVV)) ;
    public fieldsConstMailResive: IBaseUsers = JSON.parse(JSON.stringify(REGISTRATION_MAILRESIVE_EPVV));
    ...
    private setRcSend = new Set()
        .add('RCSEND_EPVV_FOR_MULTIPOINT_DOCUMENTS_SEND_RADIO')
        .add('RCSEND_EPVV_RESOLUTIONS_RADIO')
        .add('RCSEND_EPVV_ADDRESSEES_RADIO');
    ...
    private setMailResive = new Set()
        .add('MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO')
        .add('MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO');
    ...
    ngOnInit() {
        ...
        this.stringRCSEND = this.userData['RCSEND_EPVV'];
        this.stringMailResive = this.userData['MAILRECEIVE_EPVV'];
        this.mapDefault = {
            RCSEND_EPVV: this.defaultValues['RCSEND_EPVV'],
            MAILRECEIVE_EPVV: this.defaultValues['MAILRECEIVE_EPVV'],
        };
    }
    ...
    initEmail() {
        this.preparedItemForInputs = this.parse_Create(this.fieldsConst.fields, 'userData', 'RCSEND_EPVV');
    ...
    initMailResive() {
        this.preparedItemForInputsMailREsive = this.parse_Create(this.fieldsConstMailResive.fields, 'userData', 'MAILRECEIVE_EPVV');
    ...
    returnClass(node, form: FormGroup): string {
        ...
        if (key === 'rec.RCSEND_EPVV_REGISTRATION_NUMBER') {
            key = 'rec.RCSEND_EPVV_DOCUMENT_AUTHOR';
        }
        if (node.isOpen) {
    ...
    subscriberFormRcSend() {
        this.form.valueChanges.subscribe(data => {
            ...
            if (dataVal['RCSEND_EPVV'] !== this.stringRCSEND || this.mapNewValue.size > 0) {
    ...
    triggerMailReceive(node: TreeItem) {
        if (node.key === 'MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT' || node.key === 'MAILRECEIVE_EPVV_TAKE_RUBRICS_RK') {
    ...
    enabelDisabelMailResive(tree: TreeItem) {
        ...
        if (tree.key === 'MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable();
            }
        }
        if (tree.key === 'MAILRECEIVE_EPVV_TAKE_RUBRICS_RK') {
            if (value) {
                const defaultValue = this.getDefaultValue('MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO', 'mailReceive');
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO'].enable();
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO'].patchValue(defaultValue === '' ? '0' : defaultValue);
            } else {
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO'].patchValue('');
                this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO'].disable();
            }
        }
    }
    ...
    setParent(node: TreeItem, val?) {
        ...
        if (node.key === 'RCSEND_EPVV_ADDRESSEES' || node.key === 'RCSEND_EPVV_RESOLUTIONS') {
            this.enabelDisabelRcSend(node);
        }
    }

    disableForm() {
        const addresInput = this.form.controls['rec.RCSEND_EPVV_ADDRESSEES'].value;
        const resolutionInput = this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS'].value;

        (!addresInput) ? this.form.controls['rec.RCSEND_EPVV_ADDRESSEES_RADIO'].disable({ emitEvent: false }) :
                         this.form.controls['rec.RCSEND_EPVV_ADDRESSEES_RADIO'].enable({ emitEvent: false });

        (!resolutionInput) ? this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS_RADIO'].disable({ emitEvent: false }) :
                             this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS_RADIO'].enable({ emitEvent: false });
    }

    disableFormMailResive() {
        const addresInput = this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT'].value;
        const resolutionInput = this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK'].value;

        (!addresInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].disable({ emitEvent: false }) :
                        this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_NOTIFY_ABOUT_REGISTRATION_OR_REFUSAL_FROM_IT_RADIO'].enable({ emitEvent: false });
        
        (!resolutionInput) ? this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO'].disable({ emitEvent: false }) : 
                             this.formMailResuve.controls['rec.MAILRECEIVE_EPVV_TAKE_RUBRICS_RK_RADIO'].enable({ emitEvent: false });
    }

    getDefaultValue(key: string, type: 'rcSend' | 'mailReceive') {
        if (type === 'rcSend') {
            if (!this.prepareDefaultForm) {
                this.prepareDefaultForm = this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_EPVV');
            }
        ...
        if (type === 'mailReceive') {
            if (!this.prepareDefaultFormMailREceive) {
                this.prepareDefaultFormMailREceive = this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_EPVV');
            }
    ...
    enabelDisabelRcSend(tree: TreeItem) {
        ...
        if (tree.key === 'RCSEND_EPVV_ADDRESSEES') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_EPVV_ADDRESSEES_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_EPVV_ADDRESSEES_RADIO'].enable();
                this.form.controls['rec.RCSEND_EPVV_ADDRESSEES_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_EPVV_ADDRESSEES_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_EPVV_ADDRESSEES_RADIO'].disable();
            }
        }

        if (tree.key === 'RCSEND_EPVV_RESOLUTIONS') {
            if (value) {
                const defaultValue = this.getDefaultValue('RCSEND_EPVV_RESOLUTIONS_RADIO', 'rcSend');
                this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS_RADIO'].enable();
                this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS_RADIO'].patchValue(defaultValue);
            } else {
                this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS_RADIO'].patchValue('');
                this.form.controls['rec.RCSEND_EPVV_RESOLUTIONS_RADIO'].disable();
            }
        }
    }
    ...
    subscriberFormMailResive() {
        this.formMailResuve.valueChanges.subscribe(data => {
            ...
            const dataVal = this.userData !== null ? this.userData : this.defaultValues;
            if (dataVal['MAILRECEIVE_EPVV'] !== this.stringMailResive) {
    ...
    emitChange() {
        const obj = {
            'rec.RCSEND_EPVV': this.stringRCSEND,
            'rec.MAILRECEIVE_EPVV': this.stringMailResive,
        };
    ...
    default() {
        this.prepareDefaultForm = this.prepareDefaultForm ? this.prepareDefaultForm : this.parse_Create(this.fieldsConst.fields, 'mapDefault', 'RCSEND_EPVV');
        this.prepareDefaultFormMailREceive = this.prepareDefaultFormMailREceive ? this.prepareDefaultFormMailREceive : this.parse_Create(this.fieldsConstMailResive.fields, 'mapDefault', 'MAILRECEIVE_EPVV');
