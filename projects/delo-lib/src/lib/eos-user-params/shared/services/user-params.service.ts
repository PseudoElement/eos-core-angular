import {
    ...
    HTML_TEMPLATE_DATA,
    HTML_TEMPLATE_DATE,
    HTML_TEMPLATE_SHORT_REPORT,
} from "./../consts/user-param.consts";
...
export class UserParamsService {
        ...
        return Promise.all([_user, _sys])
            .then(([user, sys]) => {
                ...
                if (this._userContext.USER_RIGHT_DOCGROUP_List) {
                    this._userContext.USER_RIGHT_DOCGROUP_List.forEach((item) =>
                        this.userRightDocgroupList.push(Object.assign({}, item))
                    );
                }
                if (this._userContext["USER_EDIT_ORG_TYPE_List"]) {
                    this.userEditOrgType = this._absRigthSer.getUserEditOrgType(
                        this._userContext["USER_EDIT_ORG_TYPE_List"]
                    );
                }
    ...
    fetchSysParams() {
        ...
        return this._pipSrv.getData(querySys).then((data) => {
            const h = {};
            data.forEach((e) => {
    ...
    ceckOccupationDueDep(dueDep: string, dep: DEPARTMENT, isn?: boolean) {
        /* проверяем прикреплино ли должностное лицо к пользователю */
        const mess: IMessage = {
            title: "Предупреждение:",
            msg: "",
            type: "warning",
        ...
        return this._pipSrv
            .getData<USER_CL>({ USER_CL: PipRX.criteries({ DUE_DEP: `${dep.DUE}` }) })
            .then((u: USER_CL[]) => {
                if (this._appContext.limitCardsUser.length > 0) {
                    if (this._appContext.limitCardsUser.indexOf(dep.DEPARTMENT_DUE) !== -1) {
                        // проверку написать на существующего пользователя
                        if (!u.length) {
                            return dep;
                        } else {
    ...
    getUserCbRoles(due: string): Promise<any> {
        ...
        return Promise.all(reqs).then((data: any[]) => {
            const asistMansData = data[0]
                .filter(
                    (el) => el.ISN_USER !== this.curentUser.ISN_LCLASSIF && (el.KIND_ROLE === 4 || el.KIND_ROLE === 5)
                )
                .map((el) => el.ISN_USER);
        ...
        data.forEach((el) => {
            if (el.KIND_ROLE === 4 || el.KIND_ROLE === 5) {
                currentCbFields.push({
                    role: KIND_ROLES_CB[el.KIND_ROLE - 1],
                    dueName: dueNames.get(el.DUE_PERSON),
                    due: el.DUE_PERSON,
                    isnRole: el.ISN_USER_ROLE,
                });
            } else if (this.asistMansStr && (el.KIND_ROLE === 1 || el.KIND_ROLE === 2 || el.KIND_ROLE === 3)) {
                currentCbFields.push({
                    role: KIND_ROLES_CB[el.KIND_ROLE - 1],
                    asistMan: this.asistMansStr,
                    isnRole: el.ISN_USER_ROLE,
                });
            } else {
    ...
    getSysTechUser(
        curUser: { oldRights: string[]; newRights: string[]; editUser: IParamUserCl } = null,
        checkedUser = this.checkedUsers
    ): Promise<any> {
        ...
        if (!curUser) {
            ...
            limitTechUser =
                curUser.oldRights[+ETypeDeloRight.SystemTechnologist] === "1" &&
                curUser.newRights[+ETypeDeloRight.SystemTechnologist] === "0";
    ...
    createShortReportHtml(data) {
        ...
        const html: string = HTML_TEMPLATE_SHORT_REPORT.replace(HTML_TEMPLATE_TITLE, htmlTitle)
        ...
        return this.getLicenseInfo().then((licenseInfo) => {
            if (licenseInfo && licenseInfo.length) {
                ...
                CONFIRM_UNAVAILABLE_SYSTEMS_AFTER_BLOCK.bodyList = [
                    "Заблокированных технологом пользователей невозможно разблокировать.",
                ];
                Object.keys(systems).forEach((key) => {
                    const system = systems[key];
                    const lic = licenseMap.get(system.id);
                    if (key !== "delowebKL" && lic && lic.max - lic.cur < 0) {
                        // Пользователей с КЛ можно зарегистрировать сколько угодно
                        hasCrowded = true;
    ...
    private _createHash() {
        ...
        this._userContext["USER_PARMS_List"].forEach((item) => {
            /*
                в рамках бага 176353, не записываю в контекст роль  пользователя , если она от -99 (по умолчанию).
                иначе в карточке пользователя она подставляется в контрол Роль , а реально ее не существует у пользака и нужно пересохрянять,
                согласовано с Телещуком
            */
            if (item.PARM_NAME === "CATEGORY" && item.ISN_USER_OWNER !== item.REAL_ISN_USER_OWNER) {
                return;
            }
