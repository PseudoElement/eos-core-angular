        ...
        TooltipDirective.prototype.show = function () {
            ...
            if (this.delay) {
                ...
                const _timer_1 = setTimeout(() => {
                    if (_this._elementRef.nativeElement.scrollWidth > 0) {
                        showTooltip();
                    }
                ...
                if (this.triggers) {
                    ...
                    for (let i = 0; i < triggers.length; i++) {
                        _tooltipCancelShowFn1.push(this._renderer.listen(this._elementRef.nativeElement, triggers[i].close, () => {
                            // console.log("TCL: EosTooltipService -> TooltipDirective.prototype.show -> triggers[i].close", triggers[i].close)
                            _closeEventAction();
                        }
                        ));
                    }
                    _this._tooltipCancelShowFn = _tooltipCancelShowFn1[0];
                }
            } else {
                showTooltip();
            }
